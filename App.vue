    ...
    <div class="row">
      ...
      >
        ...
        <div class="d-flex flex-column align-items-end mb-3">
          <small
            >Created At: {{ new Date(selected.createdAt).toDateString() }}
...
//compositionAPI를 사용하면 데이터와 관련 로직을 한곳에서 관리할수있음
//compositionAPI를 사용하기위해 필수 set(){ } ㄱㄱ
export default {
  setup() {
    const {
      text,
    ...
    const deleteToDoAndResetSelected = (createAt) => {
      deleteToDo(createAt);
...
.task-completed {
  ...
  //       (toDo) => toDo.text.trim() === this.text.trim()
  //     );
  //   },
  //   filteredList() {
